// SEE https://github.com/sleepy-monax/vtconsole
...
vtconsole_t *vtconsole(int64_t width, int64_t height, vtc_paint_handler_t on_paint, vtc_cursor_handler_t on_move) {
...
void vtconsole_clear(vtconsole_t *vtc, int64_t fromx, int64_t fromy, int64_t tox, int64_t toy) {
	for (int64_t i = fromx + fromy * vtc->width; i < tox + toy * vtc->width; i++) {
...
void vtconsole_scroll(vtconsole_t *vtc, int64_t lines) {
	if (lines == 0) return;

	lines = lines > vtc->height ? vtc->height : lines;

	// Scroll the screen by number of $lines.
	for (int64_t i = 0; i < ((vtc->width * vtc->height) - (vtc->width * lines)); i++) {
	...
	for (int64_t i = ((vtc->width * vtc->height) - (vtc->width * lines)); i < vtc->width * vtc->height; i++) {
		vtcell_t *cell = &vtc->buffer[i];
		cell->attr = VTC_DEFAULT_ATTR;
		cell->c = ' ';
...
void vtconsole_csi_cup(vtconsole_t *vtc, vtansi_arg_t *stack, int64_t count) {
...
void vtconsole_csi_ed(vtconsole_t *vtc, vtansi_arg_t *stack, int64_t count) {
	(void)(count);
...
void vtconsole_csi_el(vtconsole_t *vtc, vtansi_arg_t *stack, int64_t count) {
	(void)(count);
...
void vtconsole_csi_sgr(vtconsole_t *vtc, vtansi_arg_t *stack, int64_t count) {
	for (int64_t i = 0; i < count; i++) {
		if (stack[i].empty || stack[i].value == 0) {
			vtc->attr = VTC_DEFAULT_ATTR;
		} else {
			int64_t attr = stack[i].value;
...
void vtconsole_write(vtconsole_t *vtc, const char *buffer, int64_t size) {
